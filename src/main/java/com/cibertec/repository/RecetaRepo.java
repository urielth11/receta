...
public interface RecetaRepo extends JpaRepository<Receta, Integer> {
	@Query(value = "SELECT * FROM receta r INNER JOIN megusta m on r.id = m.receta_id WHERE m.usuario_id = ?1 and m.activo = 1 and r.activo = 1", nativeQuery = true)
	...
	@Query(value = "SELECT * FROM receta r WHERE r.activo = 1", nativeQuery = true)
